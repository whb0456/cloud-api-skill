...
- 使用 **docker load** 命令，载入镜像文件。
  ...
  ```
- 进入source/backend\_service/sample/src/main/resources 下，修改后台配置文件“application.yml“，修改配置文件中的mysql配置、mqtt配置、redis配置以及对象存储服务器配置。
- 进入source/nginx/front\_page/src/api/http 下，修改前端配置文件”config.ts“，[填上在开发者网站申请的 APP ID、APP Key 和 APP License。](https://developer.dji.com/cn/user/apps/#all)

![](https://terra-1-g.djicdn.com/fee90c2e03e04e8da67ea6f56365fc76/SDK 文档/CloudAPI/appinformation.jpeg)
...
- **进入update\_front.sh 文件的目录下，构建前后端镜像**
