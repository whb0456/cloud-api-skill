...
**Method:** obstacle_avoidance_notify

**Data:**

| Column | Name | Type | constraint | Description |
| --- | --- | --- | --- | --- |
| wayline_uuid | 航线 ID | text |  | 航线任务 ID |
| flight_id | 任务 ID | text |  | 指令飞行 / 航线任务 ID |
| obstacles | 避障记录列表 | array | {"size": -, "item_type": struct} | 避障事件记录列表 |
| »id | 障碍物 ID | int |  | 在当架次中的唯一索引 |
| »type | 避障点类型 | enum_int | {0:"绕行开始（飞控映射到航线上）",1:"绕行结束（飞控映射到航线上）",2:"避障刹停（真实物理坐标）"} | 避障点类型 |
| »timestamp | 发生时间戳 | int64 |  | 避障发生时间戳（ms） |
| »latitude | 纬度 | double | {"max":90,"min":-90} | 避障发生位置纬度 |
| »longitude | 经度 | double | {"max":180,"min":-180} | 避障发生位置经度 |
| »height | 高度 | double | {"unit_name":"米 / m"} | 避障发生位置 WGS84 椭球高 |
| »wayline_id | 航段 ID | int |  | 避障所在航段（多航线任务区分使用） |
| »waypoint_index | 航点索引 | int |  | 避障所在航段航点索引（首航点所在航段为 0） |
| is_final_report | 是否最后一条避障记录 | boolean |  | 标识该条消息是否为当前避障记录组的最后一条消息。true 表示该避障事件组已上报完成。 |
...
```
{
  ...
  "need_reply": 1,
  "data": {
    "wayline_uuid": "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxx",
